...
async function showSplashScreen() {
    ...
    gifImage.src = 'https://raw.githubusercontent.com/rssantos013/KhanAcademy.Hack/a266ba68bbbe4d0735bed16149970dcf7d3e98a4/gif.gif';
...
async function hideSplashScreen() {
    ...
    setTimeout(() => splashScreen.remove(), 8500);
};
...
function setupMenu() {
    ...
    function setupWatermark() {
        ...
        if (device.mobile) watermark.style.left = '1%'
        watermark.innerHTML = `<span style="text-shadow: -1px 0.5px 0 #000000;">KhanAcademy</span> <span style="color:black; padding-left:2px; font-family: Arial, sans-serif; font-size:10px">${ver}</span>`;
    ...
    function setupDropdown() {
        Object.assign(dropdownMenu.style, {
            position: 'absolute', top: '100%', left: '0', width: '160px', backgroundColor: 'rgba(0,0,0,1)',
    ...
    function setupStatusPanel() {
        ...
        const update = async () => statsPanel.innerHTML = `
            <span style="text-shadow: -1px 0.5px 0 #p, -2px 0px 0 #2f672e;">Bypass - @r.ssantos013 😈</span>
        `;
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ "🔥 Khan Academy Hackeado!", "🤍 Bypass - @r.ssantos013", "☄️ Atualizado - 29/10/2024 " ];
    ...
    function changeBannerText() {
        const phrases = [ "[😈] Hackeado Com Sucesso", "[😈] Bypass no topo!.", "[😈] @r.ssantos013", "[😈] Obrigado por utilizar" ];
        setInterval(() => {
        ...
        while (true) {
            if(features.autoAnswer&&features.questionSpoof){
                ...
                classToCheck.forEach(async (q) => {
                    ...
                    if(element&&element.textContent=='Mostrar resumo') { sendToast("🎉 Exercício concluido com sucesso!", 3000); playAudio('https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/4x5g14gj.wav'); }
                });
            }
...
/* Inject */
if (!/^https?:\/\/pt\.khanacademy\.org/.test(window.location.href)) { alert("❌ Erro ao Injetar\n\nVocê precisa executar no site do Khan Academy! (https://pt.khanacademy.org/)"); window.location.href = "https://pt.khanacademy.org/";};
